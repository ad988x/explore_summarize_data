...
The graph is right-skewed.  The data is mostely between 7 & 8, with a few outliers.
...
The bulk of the data is between 0.3 & 0.7 with a few outliers between 1.0 and 1.58.  The mean of the volatile acidity is 0.5278.
...
The bulk of the observations land between 0 and 0.5 with the bulk of the data at/around 0, with a very small percentage of outliers between 0.8 & 1.
...
This graph is very similar to the sugar graph, with the bulk of the observations at .1 and extreme outliers.
...
This is similar to the 2 previous charts with the observations being right skewed with a few outliers.  
...
This graph is another that is right skewed with bulk of the data between 10 & 30 with a small number of extreme outliers.
...
The Sulphates graph is skewed to the right, with a few outliers.  This is the like some of the previous graphs we have seen.
...
The range of quality is 3 through 8, but most of the ranking is 5 and 6, with the others as smaller pieces of the ranking. 
...
ggplot(aes(x = residual.sugar, y = alcohol), data = rw) + 
  geom_point(aes(color = quality), alpha = 0.5, size = 4, position = 'jitter') 
```
